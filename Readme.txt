...
RightBarButtonItem -> переход на SearchViewController. Из-за выбранных мною цветов UI пришлось делать кастомный SearchBar (oh god why). Запрос к серверу (func updateSearchResults(text:)) осуществляется по мере набора символов в строку поиска (searchBar.textField) с использованием функции NSObject.cancelPreviousPerformRequests 
